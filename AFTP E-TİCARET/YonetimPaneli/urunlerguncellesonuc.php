...
if ( ( $GelenUrunMenusu != "" )and( $GelenUrunAdi != "" )and( $GelenUrunFiyati != "" )and( $GelenParaBirimi != "" )and( $GelenKdvOrani != "" )and( $GelenKargoUcreti != "" )and( $GelenUrunAciklamasi != "" )and( $GelenVaryantBasligi != "" )and( $GelenVaryantAdi1 != "" )and( $GelenStokAdedi1 != "" ) ) {
  ...
  if ( $UrunKontrol > 0 ) {
    ...
    if ( $MenuTuruKaydi[ "UrunTuru" ] == "Kayisi" ) {
      $ResimKlasoru = "UrunResimleri/kayisi/";
    } elseif ( $MenuTuruKaydi[ "UrunTuru" ] == "Kuruyemis" ) {
      $ResimKlasoru = "UrunResimleri/kuruyemis/";
    } else {
      $ResimKlasoru = "UrunResimleri/bos/";
    }
    ...
    if ( $MenuTuruKontrol > 0 ) {
      ...
      $VaryantlarSorgusu = $VeritabaniBaglantisi->prepare( "SELECT * FROM urunvaryanlari WHERE UrunId = ?" );
      ...
      foreach ( $VaryantBilgisi as $Varyant ) {
        $VaryantIsimDizisi[] = $Varyant[ "VaryanAdi" ];
      }

      if ( array_key_exists( 0, $VaryantIsimDizisi ) ) {
        if ( ( $GelenVaryantAdi1 != "" )and( $GelenStokAdedi1 != "" ) ) {
          $BirinciVaryantGuncellemeSorgusu = $VeritabaniBaglantisi->prepare( "UPDATE urunvaryanlari SET VaryanAdi = ?, StokAdedi = ? WHERE UrunId = ? AND VaryanAdi = ? LIMIT 1" );
